   ...
   this.MhandleFileError= function(evt) {
	...
	this.gametrack = function(){// generates track based on the range of sample set to a pattern -- you can adjust the pattern as like
		 	...
		 	for(var i = 0; i < 5; i++){
				...
				//set the beat for the sound within the loop
				if(i== 4){
					var mdrange= Math.round(Math.random() *3+1);// select between range of sample  
				...
				if(i== 2){
					var hrange= Math.round(Math.random() *5 + 4); 
...
var manifest = [//full list of all sample audio-- !lack of range
       {src:"K02Toypiano-C4.wav", id:"fall"},
       {src:"SYNTHPAD004_HPHOP_90_A_SC2(R).wav", id:"0"},
	   {src:"K02Toypiano140D-01.wav", id:"1"},
	   {src:"PERCLOOP009_INDST_140_X_SC2.wav", id:"2"},
	   {src:"SYNTHPAD003_HPHOP_90_A_SC2(R).wav", id:"3"},
	   {src:"FX045_TEKNO_140_X_SC4.wav", id:"4"},
	   {src:"ORGAN017_POP_90_E_SC4(L).wav", id:"5"}
