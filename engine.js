...
Engine.prototype.translateWorld = function(t) {
   ...
   this.MhandleFileError= function(evt) {
	...
	this.gametrack = function(){// generates track based on the range of sample set to a pattern -- you can adjust the pattern as like
		 	...
		 	for(var n = 0; n < 16; n++ ){
				if(n == Math.round(Math.random()*15)){
					...
					var note = createjs.Sound.createInstance(lowrange.toString());
					note.setVolume(0.1); 
					note.play();
				}
				//set the beat for the sound within the loop - game sound 
				if( n == Math.round(Math.random()*6) ){
					var mdrange= Math.round(Math.random()*3);// select between range of sample  
					var note2 = createjs.Sound.createInstance(mdrange.toString());// sounds is set to play 
					note2.setVolume(0.2); //adjust the sounds volume to mix -- change as needed 
					note2.play();//play sound	
				}
				if( n == Math.round(Math.random()*10)){
					var hrange= Math.round(Math.random()*7 + 4); 
					var note3 = createjs.Sound.createInstance(hrange.toString());
					 note3.setVolume(0.1); 
					 note3.play();
				}
			}
...
var manifest = [//full list of all sample audio-- !lack of range
		//8bit loop
       {src:"CM139_Loop01(90BPM).wav", id:"0"},
	   {src:"CM139_Loop02(90BPM).wav", id:"1"},
	   {src:"CM139_Loop03(90BPM).wav", id:"2"},
	   {src:"CM139_Loop19(130BPM).wav", id:"3"},
		//8bit loop
	   {src:"CM139_Loop12(130BPM).wav", id:"4"},
	   {src:"CM139_Loop13(130BPM).wav", id:"5"},
	   {src:"CM139_Loop13(130BPM).wav", id:"6"},
	   {src:"CM139_Loop08(130BPM).wav", id:"7"},
		//8bit loop 
	   {src:"CM139_Loop05(90BPM).wav", id:"8"},
	   {src:"CM139_Loop17(130BPM).wav", id:"9"},
	   {src:"CM139_Loop18(130BPM).wav", id:"10"},
	   {src:"CM139_Loop06(130BPM).wav", id:"11"},
		//8bit sound fx
	   {src:"CM139_FX16.wav", id:"jumfx"},
	   {src:"CM139_FX24.wav", id:"damagefx"}
